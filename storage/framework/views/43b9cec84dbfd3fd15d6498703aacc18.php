...
<div x-data="{ showAddProject: false, openLocationFor: null }">
    <div class="border-b border-slate-200 bg-slate-50/80 px-4 py-3">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                <p class="text-[13px] font-semibold text-navy-900">Project Monitoring Work Plan</p>
                <p class="text-[11px] text-slate-500">July <?php echo e($fyParts[0] ?? ''); ?> to June <?php echo e($fyParts[1] ?? ''); ?> · same data as <a href="<?php echo e(route('projects.index')); ?>" class="font-medium text-brand-600 hover:underline">Projects</a></p>
            </div>
            <button
                type="button"
        ...
        <div x-show="showAddProject" x-cloak class="mt-3 rounded-xl border border-slate-200 bg-white p-4">
            <p class="mb-3 text-[12px] font-medium text-navy-900">New monitoring project</p>
            <form method="POST" action="<?php echo e(route('annual-audit.projects.store')); ?>" x-data="{ locations: [{ name: '', division: '' }] }">
                <?php echo csrf_field(); ?>
                <input type="hidden" name="status" value="active">
                <input type="hidden" name="has_project_monitoring" value="1">
...
<?php endif; ?>
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 text-[12px] text-slate-600">
                            <input type="checkbox" name="has_project_audit" value="1" class="rounded border-slate-300 text-brand-600">
                            Also include in Project Audit
                        </label>
                    </div>
                </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full border-collapse text-left">
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $projectGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <?php if($group['rows']->isEmpty()): ?>
                        <tr class="text-[12px]">
                            ...
                            <td class="border border-slate-200 px-2 py-2 text-center">
                                <form method="POST" action="<?php echo e(route('annual-audit.projects.destroy', $group['project_id'])); ?>" onsubmit="return confirm('Delete this project and all its locations?')">
                                    <?php echo csrf_field(); ?>
                    ...
                    <?php else: ?>
                        <?php $__currentLoopData = $group['rows']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr
                                class="text-[12px]"
                            ...
                            >
                                ...
                                <td class="border border-slate-200 px-3 py-1.5">
                                    <div class="flex items-center justify-between gap-2">
                                        <span class="text-slate-700"><?php echo e($row['location']); ?></span>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.audit-month-mark','data' => ['active' => (bool) $active,'manual' => (bool) ($row['manual'][$monthIndex] ?? false),'editable' => $canEditSchedule,'category' => $row['category'],'schedulableType' => $row['schedulable_type'],'schedulableId' => $row['id'],'monthIndex' => $monthIndex,'tab' => 'project_monitoring']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['active' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute((bool) $active),'manual' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute((bool) ($row['manual'][$monthIndex] ?? false)),'editable' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($canEditSchedule),'category' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($row['category']),'schedulable-type' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($row['schedulable_type']),'schedulable-id' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($row['id']),'month-index' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($monthIndex),'tab' => 'project_monitoring']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
                                ...
                                <?php if($index === 0): ?>
                                    <td rowspan="<?php echo e($group['rows']->count()); ?>" class="border border-slate-200 px-2 py-2 align-middle">
                                        <div class="flex flex-col items-center gap-1">
                                            ...
                                            <form method="POST" action="<?php echo e(route('annual-audit.projects.destroy', $group['project_id'])); ?>" onsubmit="return confirm('Delete project <?php echo e($group['project']); ?> and all locations?')">
                                                <?php echo csrf_field(); ?>
                                                <?php echo method_field('DELETE'); ?>
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="<?php echo e(count($months) + 5); ?>" class="border border-slate-200 px-4 py-10 text-center text-[12px] text-slate-400">
                            No monitoring projects yet. Click <span class="font-medium text-navy-800">Add Project</span> to match your Excel work plan.
                        </td>
                    </tr>
                <?php endif; ?>
            </tbody>
    ...
    <p class="border-t border-slate-100 px-4 py-2 text-[11px] text-slate-500">
        Green cells = planned monitoring visit. Use <span class="font-medium text-slate-700">Remove</span> on a location or <span class="font-medium text-slate-700">Delete</span> on a project to undo mistakes.
    </p>
</div>
<?php /**PATH C:\xampp\htdocs\Bynnas-Audit\resources\views/annual-audit/partials/project-monitoring.blade.php ENDPATH**/ ?>