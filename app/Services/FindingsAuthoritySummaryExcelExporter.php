...
class FindingsAuthoritySummaryExcelExporter
{
    ...
    protected function paintUnderheadingSummary(Worksheet $sheet, array $groups, string $periodLabel): void
    {
        ...
        $headers = ['Heading', 'Sub-heading', 'Code', 'Indicator', 'Amount', 'Sample size', 'Irregularities', 'Irregularity %', 'Total branch', 'Branch name'];
        ...
        foreach ($groups as $group) {
            foreach ($group['rows'] as $row) {
                ...
                $sheet->setCellValue('J'.$r, $row['branches'] !== '' ? $row['branches'] : '—');
        ...
        if ($r === 4) {
            ...
            $sheet->getStyle('A4:J'.($r - 1))->applyFromArray([
                'borders' => [
                    'allBorders' => ['borderStyle' => Border::BORDER_THIN, 'color' => ['rgb' => 'E2E8F0']],
                ],
            ]);
            $sheet->setAutoFilter('A3:J'.($r - 1));
        }
