...
@if (($page12_section_title ?? '') !== '')
    <p class="bold center" style="margin:4mm 0 3mm;font-size:12px;text-decoration:underline;">{{ $page12_section_title }}</p>
@endif

@foreach ($page12Findings ?? [] as $finding)
    @php
            ...
            <tbody>
                @foreach ($finding['stockRows'] ?? [] as $row)
                    <tr>
                        ...
                        <td class="center">{{ $row['purchase_date_voucher'] ?? '' }}</td>
