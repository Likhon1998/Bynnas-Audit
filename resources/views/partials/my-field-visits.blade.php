        ...
        <div class="divide-y divide-sky-100/80">
            @foreach ($todayActiveList as $row)
                <div class="flex flex-wrap items-center justify-between gap-3 px-4 py-3.5">
                        <div class="min-w-0 flex-1">
                            <div class="flex flex-wrap items-center gap-1.5">
                                ...
                                @if (! empty($row['is_solo']))
                                    <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">Solo</span>
                                @elseif (! empty($row['team_label']))
                                    <span class="rounded-full bg-violet-50 px-2 py-0.5 text-xs font-semibold text-violet-800 ring-1 ring-violet-200">{{ $row['team_label'] }}</span>
                                @endif
                            </div>
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        <span class="rounded-full border px-2 py-0.5 text-xs font-semibold capitalize {{ $row['tone']['bg'] }} {{ $row['tone']['text'] }}">{{ $row['status_label'] }}</span>
            ...
            @foreach ($todayDoneList as $row)
                <div class="flex flex-wrap items-center justify-between gap-3 bg-teal-50/40 px-4 py-3">
                    ...
                    <a href="{{ $row['execution_url'] }}" class="text-[13px] font-semibold text-teal-800 hover:underline">View</a>
                </div>
            @endforeach
        </div>
    @endif
...
@if ($showMonthStats)
    <div class="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
            ...
            <p class="text-xs text-slate-500">{{ $monthLabel ?? 'Visits' }}</p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
            <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Shakhas</p>
            <p class="mt-0.5 text-[18px] font-semibold tabular-nums text-navy-900">{{ number_format($stats['shakhas_month'] ?? 0) }}</p>
            <p class="text-xs text-slate-500">Branches this month</p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
            ...
            <p class="text-xs text-slate-500">{{ $stats['month_completion_pct'] ?? 0 }}% of month</p>
        </div>
        <div class="rounded-xl border border-slate-200 bg-white px-3 py-2.5">
...
<div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 bg-slate-50/80 px-3.5 py-2.5">
        <div>
            ...
            <p class="text-xs text-slate-500">{{ $monthLabel ?? 'This month' }} · {{ count($allocations) }} allocation{{ count($allocations) === 1 ? '' : 's' }}</p>
        </div>
        @if ($visitsUrl)
            <a href="{{ $visitsUrl }}" class="text-[13px] font-semibold text-brand-600 hover:underline">Full calendar →</a>
        @endif
    </div>
        ...
        @forelse ($allocations as $row)
            <div class="flex flex-wrap items-center justify-between gap-2 px-3.5 py-2.5 {{ ! empty($row['is_today']) ? 'bg-sky-50/70' : '' }}">
                <div class="min-w-0 flex-1">
                    <div class="flex flex-wrap items-center gap-1.5">
                        ...
                        @if (! empty($row['is_today']))
                            <span class="rounded bg-sky-600 px-1.5 py-0.5 text-xs font-bold uppercase tracking-wide text-white">Today</span>
                        @endif
                        @if (! empty($row['risk']))
                            <x-shakha-risk-badge :category="$row['risk']" size="xs" />
                        @endif
                        @if (! empty($row['is_solo']))
                            <span class="rounded-full bg-slate-100 px-1.5 py-0.5 text-xs font-semibold text-slate-600 ring-1 ring-slate-200">Solo</span>
                        @elseif (! empty($row['team_label']))
                            <span class="rounded-full bg-violet-50 px-1.5 py-0.5 text-xs font-semibold text-violet-800 ring-1 ring-violet-200">{{ $row['team_label'] }}</span>
                        @endif
                    </div>
                ...
                <div class="flex shrink-0 items-center gap-1.5">
                    <span class="rounded-full border px-2 py-0.5 text-xs font-semibold capitalize {{ $row['tone']['bg'] }} {{ $row['tone']['text'] }}">{{ $row['status_label'] }}</span>
                    <a href="{{ $row['execution_url'] }}" class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-[13px] font-semibold text-slate-700 hover:bg-slate-50">Open</a>
                </div>
            </div>
        @empty
