...
>
    <div class="border-b border-slate-200 bg-slate-50/80 px-3 py-2.5">
        <div class="flex flex-wrap items-start justify-between gap-2">
            <div>
                <p class="text-[12px] font-semibold text-navy-900">Headquarters (HQ) Work Plan</p>
            ...
            <div class="flex flex-wrap items-center gap-2">
                <a
                    href="{{ route('annual-audit.export', ['mode' => 'hq', 'fy' => $plan->fy_label]) }}"
        ...
        <div x-show="showAdd" x-cloak class="mt-3 rounded-xl border border-slate-200 bg-white p-4">
            <form method="POST" action="{{ route('annual-audit.hq.store') }}" class="flex flex-wrap items-end gap-2">
                @csrf
        ...
        <table class="min-w-full border-collapse text-left">
            ...
            <tbody>
                @forelse ($rows as $row)
                    ...
                    >
                        <td class="border border-slate-200 px-2 py-1.5 text-center text-slate-500">{{ $row['sl'] }}</td>
                        <td class="border border-slate-200 px-3 py-1.5 font-medium text-navy-900">{{ $row['name'] }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="16" class="border border-slate-200 px-3 py-6 text-center text-[12px] text-slate-500">
            ...
            @if ($rows->isNotEmpty())
                <tfoot>
                    ...
                    <tr class="bg-emerald-50/50 text-[12px]">
