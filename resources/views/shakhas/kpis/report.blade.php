<x-app-layout>
    ...
    <div class="px-4 py-5 lg:px-6 print:px-0">
        {{-- Document toolbar (hidden when printing) --}}
        <div class="mb-4 flex flex-wrap items-start justify-between gap-3 print:hidden">
            ...
            <div class="flex flex-wrap items-center gap-1.5">
                <a href="{{ route('shakhas.kpis.create', ['shakha' => $shakha, 'month' => $month, 'year' => $year]) }}" class="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-[12px] font-medium text-slate-600 hover:bg-slate-50">Edit raw data</a>
        ...
        @if (session('status'))
            <div class="mb-3 rounded-lg border border-emerald-100 bg-emerald-50 px-3 py-2 text-[12px] text-emerald-800 print:hidden">{{ session('status') }}</div>
        @endif
        ...
        <div class="mb-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card print:rounded-none print:border-slate-300 print:shadow-none">
            <div class="border-b border-slate-200 bg-gradient-to-r from-slate-50 via-white to-sky-50 px-6 py-5 print:bg-white">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div class="text-right text-[12px]">
                        <p class="font-semibold text-navy-900">{{ $period_label }}</p>
                        <p class="mt-0.5 text-slate-500">Generated {{ now()->format('d M Y, h:i A') }}</p>
                        <p class="mt-0.5 text-slate-400">Document ref: KPI-{{ $shakha->id }}-{{ $year }}{{ str_pad((string) $month, 2, '0', STR_PAD_LEFT) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="grid gap-0 border-b border-slate-100 sm:grid-cols-4">
                <div class="border-b border-slate-100 px-5 py-3 sm:border-b-0 sm:border-r">
                    ...
                    <p class="mt-0.5 text-[13px] font-semibold text-navy-900">{{ $shakha->name }}</p>
                </div>
                <div class="border-b border-slate-100 px-5 py-3 sm:border-b-0 sm:border-r">
                    <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Area</p>
                    <p class="mt-0.5 text-[13px] font-medium text-slate-700">{{ $shakha->area?->name ?? '—' }}</p>
                </div>
                <div class="border-b border-slate-100 px-5 py-3 sm:border-b-0 sm:border-r">
                    <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Division</p>
                    <p class="mt-0.5 text-[13px] font-medium text-slate-700">{{ $shakha->area?->division ?? '—' }}</p>
                </div>
                <div class="px-5 py-3">
                    <p class="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Branch code</p>
                    <p class="mt-0.5 text-[13px] font-medium text-slate-700">{{ $shakha->code ?: '—' }}</p>
                </div>
            </div>
        ...
        {{-- Section cards for readable document layout --}}
        <div class="mb-4 space-y-3 print:space-y-2">
            @foreach ($sections as $section)
                @if ($section['title'] === 'Identity')
                    @continue
                @endif
                    ...
                    <div class="grid gap-0 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                        @foreach ($section['columns'] as $col)
                            <div class="border-b border-r border-slate-50 px-4 py-2.5">
                                <p class="text-[10px] font-medium text-slate-400">{{ $col['label'] }}</p>
                                <p class="mt-0.5 whitespace-nowrap text-[13px] font-semibold tabular-nums text-navy-900">{{ $fmt($col) }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        {{-- Wide scrollable master table (57-style columns) --}}
        <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-card print:shadow-none">
            <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-3">
                <div>
                    ...
                    <p class="mt-0.5 text-[11px] text-slate-500">Scroll horizontally · first column stays locked · {{ count($flat) }} indicators</p>
                </div>
                <span class="rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500 print:hidden">{{ count($flat) }} columns</span>
            </div>
                            ...
                            @foreach ($flat as $col)
                                <th
                                    class="whitespace-nowrap px-3 py-2.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500
                                        {{ ($col['key'] ?? '') === $stickyKey ? 'sticky left-0 z-20 bg-slate-50 shadow-[2px_0_4px_-2px_rgba(15,23,42,0.12)]' : '' }}"
                                >
                                    {{ $col['label'] }}
                                </th>
                            @endforeach
                            ...
                            @foreach ($flat as $col)
                                <td
                                    class="whitespace-nowrap px-3 py-2.5 tabular-nums text-slate-700
                                        {{ ($col['key'] ?? '') === $stickyKey ? 'sticky left-0 z-10 bg-white font-semibold text-navy-900 shadow-[2px_0_4px_-2px_rgba(15,23,42,0.12)]' : '' }}"
                                >
                                    {{ $fmt($col) }}
                                </td>
                            @endforeach
        ...
        <div class="mt-4 rounded-xl border border-slate-200 bg-white px-5 py-4 print:break-inside-avoid">
            ...
            <div class="mt-4 flex flex-wrap justify-between gap-4 border-t border-slate-100 pt-4 text-[11px] text-slate-400">
                ...
                <p>Bynnas Audit · {{ $shakha->name }} · {{ $period_label }}</p>
            </div>
        </div>
            ...
            <div class="mt-4 print:hidden">
                ...
                <div class="flex flex-wrap gap-1.5">
                    @foreach ($periods as $period)
                        <a
                            href="{{ route('shakhas.kpis.show', [$shakha, $period->report_month, $period->report_year]) }}"
                            class="rounded-full border px-2.5 py-1 text-[11px] font-medium
                                {{ $period->report_month === $month && $period->report_year === $year
