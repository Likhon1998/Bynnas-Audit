<x-app-layout>
    ...
    >
        <div class="mb-4 flex flex-wrap items-center justify-between gap-2.5">
            <div>
                ...
                <p class="mt-0.5 text-[11px] text-slate-500">
                    Manage branches across areas and divisions · {{ $rows->count() }} total · KPI FY {{ $fyLabel }}
                </p>
            </div>
        ...
        <div class="mb-3 rounded-xl border border-sky-100 bg-sky-50 px-3.5 py-2.5 text-[12px] text-sky-900">
            ...
            Enter annual KPI for FY {{ $fyLabel }} first. Until then, Risk stays locked and the row shows KPI Not ready.
        </div>

        {{-- Risk tabs (client-side, no reload) --}}
            ...
            @foreach ($tabs as $tab)
                ...
                >
                    ...
                    <span
                        ...
                        :class="riskTab === @js($tab['key']) ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-500'"
                    >{{ $tab['count'] }}</span>
                </button>
            @endforeach
        </div>
        ...
        <div class="mb-3 grid gap-2 rounded-xl border border-slate-100 bg-white p-3 shadow-card sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div>
                <label class="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-slate-400">KPI ({{ $fyLabel }})</label>
        ...
        <div class="mb-2 flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-500">
            <p>Showing <span class="font-semibold text-navy-900" x-text="visibleCount"></span> of {{ $rows->count() }}</p>
