...
@foreach ($findings as $fIndex => $finding)
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page17Findings.{{ $fIndex }}.criteria" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page17Findings.{{ $fIndex }}.observation" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[13px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage17StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage17StatsRow({{ $fIndex }})" class="mb-[2mm] text-[13px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif
    ...
    @if ($detailType === 'savings_partial_adjust')
        ...
        @if ($editable)
            <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
        @endif
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-[8px]' : 'w-full border-collapse text-xs' }} min-w-full">
                ...
                <tbody>
                    @foreach (($finding['savingsAdjustRows'] ?? []) as $rowIndex => $row)
                        <tr>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.samity_no" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]">
                                @else
                                    {{ $row['samity_no'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.member_name_id" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]">
                                @else
                                    {{ $row['member_name_id'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.adjust_date" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]" />
                                @else
                                    {{ $row['adjust_date'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.adjust_amount" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]">
                                @else
                                    {{ $row['adjust_amount'] ?? '' }}
                                @endif
                            </td>
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['savingsAdjustRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage17SavingsAdjustRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage17SavingsAdjustRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Savings adjust row</button>
        @endif
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[13px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.risk" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <table class="{{ $tableClass }} mb-[5mm]">
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <textarea wire:model.live="page17Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <textarea wire:model.live="page17Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[13px]" />
                    @else
