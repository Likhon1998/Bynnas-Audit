...
@foreach ($findings as $fIndex => $finding)
        ...
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }} w-[11%] text-center font-bold align-top">
                    @if ($editable)
                        <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.title" class="w-full border-0 bg-sky-50/40 text-center font-bold">
                    @else
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        @include('livewire.partials.audit-indicator-combobox', [
                            ...
                            'wireKey' => 'p17-ind-'.$fIndex.'-'.(int) ($finding['indicator_id'] ?? 0).'-'.md5((string) ($finding['body'] ?? '')),
                        ])
                        <div class="mt-1 flex flex-wrap items-center gap-2 text-[11px]">
                            <span class="font-semibold">টাকার পরিমাণ:</span>
                            <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.amount" class="inline-input min-w-[100px]">
                        </div>
                    @else
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page17Findings.{{ $fIndex }}.criteria" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[11px]"></textarea>
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page17Findings.{{ $fIndex }}.observation" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[11px]"></textarea>
        ...
        <x-audit-excel-paste-zone
            :path="'page17Findings.' . $fIndex . '.statsRows'"
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-[9px]' : 'w-full border-collapse text-[10.5px]' }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[11px]" />
                                    @else
                                        <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[11px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage17StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-[10px] text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage17StatsRow({{ $fIndex }})" class="mb-[2mm] text-[11px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif

    @php $detailType = (string) ($finding['detail_type'] ?? 'none'); @endphp
    @if ($detailType === 'savings_partial_adjust')
        <p class="mb-[1mm] font-semibold">{{ $finding['detail_intro'] ?? 'বিস্তারিত নিম্নে দেওয়া হল:' }}</p>
        @if ($editable)
            <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[11px]">
        @endif

        <div class="mb-[2mm] overflow-x-auto">
                    @if ($editable)
            <x-audit-excel-paste-zone
                :path="'page17Findings.' . $fIndex . '.savingsAdjustRows'"
                :columns="['samity_no', 'member_name_id', 'adjust_date', 'adjust_amount']"
                hint="Savings adjust paste"
            />
        @endif
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-[8px]' : 'w-full border-collapse text-[9px]' }} min-w-full">
                ...
                <tbody>
                    @foreach (($finding['savingsAdjustRows'] ?? []) as $rowIndex => $row)
                        <tr>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.samity_no" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[8px]">
                                @else
                                    {{ $row['samity_no'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.member_name_id" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[8px]">
                                @else
                                    {{ $row['member_name_id'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.adjust_date" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[8px]" />
                                @else
                                    {{ $row['adjust_date'] ?? '' }}
                                @endif
                            </td>
                            <td class="{{ $cellPad }} text-center">
                                @if ($editable)
                                    <input type="text" wire:model.live="page17Findings.{{ $fIndex }}.savingsAdjustRows.{{ $rowIndex }}.adjust_amount" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[8px]">
                                @else
                                    {{ $row['adjust_amount'] ?? '' }}
                                @endif
                            </td>
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['savingsAdjustRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage17SavingsAdjustRow({{ $fIndex }}, {{ $rowIndex }})" class="text-[10px] text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage17SavingsAdjustRow({{ $fIndex }})" class="mb-[3mm] text-[11px] font-medium text-[#2b579a]">+ Savings adjust row</button>
        @endif
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[11px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.risk" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[11px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[11px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page17Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[11px]"></textarea>
    ...
    <table class="{{ $tableClass }} mb-[5mm]">
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <textarea wire:model.live="page17Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[11px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <textarea wire:model.live="page17Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[11px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page17Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[11px]" />
                    @else
