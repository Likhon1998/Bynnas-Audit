...
@foreach ($findings as $fIndex => $finding)
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        @include('livewire.partials.audit-indicator-combobox', [
                            ...
                            'wireKey' => 'p14-ind-'.$fIndex.'-'.(int) ($finding['indicator_id'] ?? 0).'-'.md5((string) ($finding['body'] ?? '')),
                        ])
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page14Findings.{{ $fIndex }}.observation" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page14Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page14Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[13px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage14StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage14StatsRow({{ $fIndex }})" class="mb-[2mm] text-[13px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif

    @if ($editable)
        <div class="mb-[2mm] flex flex-wrap items-center gap-3">
            <label class="text-[13px] font-semibold text-slate-600">বিস্তারিত ধরন:</label>
            <select wire:model.live="page14Findings.{{ $fIndex }}.detail_type" class="rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
    ...
    @if ($detailType === 'passbook_installment')
        ...
        @if ($editable)
            <input type="text" wire:model.live="page14Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
        @endif
...
<table class="{{ $tableClass }} min-w-full">
                        ...
                        <tr>
                            @foreach ($passbookFields as $field)
                                <td class="{{ $cellPad }} text-center">
                                    @if ($editable)
                                        @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page14Findings.{{ $fIndex }}.passbookRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-xs" />
                                    @else
                                        <input type="text" wire:model.live="page14Findings.{{ $fIndex }}.passbookRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-xs">
                                    @endif
                                    @else
                                        {{ $row[$field] ?? '' }}
                                    @endif
                            ...
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['passbookRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage14PassbookRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage14PassbookRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Passbook row</button>
        @endif
...
<table class="{{ $tableClass }} min-w-full">
                        ...
                        <tr>
                            @foreach (['sl_no', 'samity_member_id', 'member_name', 'disbursement_sector', 'disbursement_date', 'actual_term', 'software_last_date', 'software_term', 'disbursed_amount', 'excess_service_charge'] as $field)
                                <td class="{{ $cellPad }} text-center">
                                    @if ($editable)
                                        @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page14Findings.{{ $fIndex }}.sufolonRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page14Findings.{{ $fIndex }}.sufolonRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-[13px]">
                                    @endif
                                    @else
                                        {{ $row[$field] ?? '' }}
                                    @endif
                            ...
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['sufolonRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage14SufolonRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage14SufolonRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Sufolon row</button>
        @endif
    @elseif (($finding['detail_intro'] ?? '') !== '' && ! $editable)
        <p class="mb-[2mm] font-semibold">{{ $finding['detail_intro'] }}</p>
    @elseif ($editable && $detailType === 'none')
        <input type="text" wire:model.live="page14Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]" placeholder="নিম্নে বিস্তারিত দেওয়া হলো:">
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[13px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page14Findings.{{ $fIndex }}.risk" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page14Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page14Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} {{ $loop->last ? '' : 'mb-[6mm]' }}">
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page14Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page14Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page14Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[13px]" />
                    @else
