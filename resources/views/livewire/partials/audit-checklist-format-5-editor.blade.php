...
<div class="overflow-hidden rounded-xl border border-slate-300 bg-white shadow-sm">
    ...
    <div class="overflow-x-auto px-2 py-3">
        <table class="min-w-[1200px] w-full border-collapse text-[13px]">
            <thead>
            ...
            <tbody>
                @foreach ($rows as $ri => $row)
                    <tr wire:key="f5-row-{{ $ri }}">
                        ...
                        <td class="border border-slate-300 p-0.5">
                            <input type="text" wire:model.live="payload.rows.{{ $ri }}.refund_date" class="h-8 w-full border-0 bg-transparent px-1 text-[13px] focus:ring-1 focus:ring-[#2b579a]">
                        </td>
                        <td class="border border-slate-300 p-0.5">
                            <input type="text" wire:model.live="payload.rows.{{ $ri }}.voucher_no" class="h-8 w-full border-0 bg-transparent px-1 text-[13px] focus:ring-1 focus:ring-[#2b579a]">
                        </td>
                        <td class="border border-slate-300 p-0.5">
                            <input type="text" wire:model.live="payload.rows.{{ $ri }}.amount" class="h-8 w-full border-0 bg-transparent px-1 text-[13px] focus:ring-1 focus:ring-[#2b579a]">
                        </td>
    ...
    <div class="border-t border-slate-200 px-3 py-2.5">
        ...
        ></textarea>
        @php
            $seedKey = \App\Services\ChecklistReportInfluenceService::formatSummarySeedKey((string) ($formatModel->code ?? 'format-5'));
