{{-- Format 4 editor. Expects: $formatModel, $definition, $payload --}}
@php
    $questions = $definition['questions'] ?? [];
    $checkCount = (int) ($definition['check_count'] ?? 12);
    $rows = $payload['rows'] ?? [];
@endphp
...
<div class="overflow-hidden rounded-xl border border-slate-300 bg-white shadow-sm">
        ...
        <table class="min-w-[1100px] w-full border-collapse text-[11px]">
            <thead>
                <tr class="bg-slate-100 text-center font-semibold text-slate-700">
                    ...
                    <th class="border border-slate-300 px-1 py-1.5 min-w-[90px]">সঞ্চয় আদায় টাকা</th>
                    <th class="border border-slate-300 px-1 py-1.5 min-w-[90px]">ঋণ আদায় টাকা</th>
            ...
            <tbody>
                @foreach ($rows as $ri => $row)
                    <tr wire:key="f4-row-{{ $ri }}">
                        ...
                        <td class="border border-slate-300 p-0.5">
                            <input type="text" wire:model.live="payload.rows.{{ $ri }}.savings_amount" class="h-8 w-full border-0 bg-transparent px-1 text-[11px] focus:ring-1 focus:ring-[#2b579a]">
                        </td>
                        <td class="border border-slate-300 p-0.5">
                            <input type="text" wire:model.live="payload.rows.{{ $ri }}.loan_amount" class="h-8 w-full border-0 bg-transparent px-1 text-[11px] focus:ring-1 focus:ring-[#2b579a]">
                        </td>
    ...
    <div class="border-t border-slate-200 px-4 py-3">
        ...
        <p class="mb-2 text-[10px] text-slate-500">Unusual (✗) marks are emphasized for the audit report. Edit after AI writes.</p>
        <textarea
        ...
        ></textarea>
        @php
            $seedKey = \App\Services\ChecklistReportInfluenceService::formatSummarySeedKey((string) ($formatModel->code ?? 'format-4'));
