...
<div class="mb-2 flex flex-wrap items-center gap-2">
    <p class="mr-auto text-[13px] font-semibold uppercase tracking-wide text-slate-500">
            ...
            @foreach ($tocRows as $idx => $row)
                ...
                <tr class="{{ $isSection ? 'bg-slate-100' : '' }}">
                    <td class="border border-slate-800 px-1 py-1">
                        <input type="text" wire:model.live="tocRows.{{ $idx }}.serial" class="finding-serial-input h-7 w-full border-0 bg-sky-50 px-1 text-center text-[13px] font-semibold focus:ring-1 focus:ring-sky-400">
                    </td>
                    <td class="border border-slate-800 px-1 py-1">
                        <input type="text" wire:model.live="tocRows.{{ $idx }}.finding" class="h-7 w-full border-0 bg-sky-50 px-1 text-[13px] {{ $isSection ? 'font-bold' : '' }} focus:ring-1 focus:ring-sky-400" placeholder="{{ $isSection ? 'Section title' : 'Finding' }}">
                    </td>
                    <td class="border border-slate-800 px-1 py-1">
                        @if ($isSection)
                    ...
                    <td class="border border-slate-800 px-0 py-0">
                        ...
                        @else
                            <select
                                ...
                                class="h-8 w-full border-0 px-1 text-[13px] font-semibold focus:ring-1 focus:ring-sky-400"
                    ...
                    <td class="border border-slate-800 px-1 py-1 text-center">
                        <button type="button" wire:click="removeTocRow({{ $idx }})" class="text-[13px] text-rose-600 hover:underline">×</button>
                    </td>
                </tr>
            @endforeach
        </tbody>
