...
>
    <button
        ...
        class="inline-flex items-center gap-1 rounded border px-2.5 py-1 text-[13px] font-semibold"
    ...
    >
        <div class="flex items-start justify-between gap-2 border-b border-sky-100 bg-sky-50 px-3 py-2.5">
            <div>
                ...
                <p class="text-xs text-sky-900/75">রঙিন নোট = কোথায় ক্লিক · নমুনা ছবি = ফলাফল কেমন দেখাবে</p>
            </div>
            <button type="button" @click="open = false" class="shrink-0 rounded border border-sky-200 bg-white px-2 py-0.5 text-[13px] font-semibold text-sky-800 hover:bg-sky-100">বন্ধ</button>
        </div>

        <ol class="space-y-2 border-b border-slate-100 bg-white px-3 py-3">
            <li class="flex gap-2.5">
                <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-600 text-[13px] font-bold text-white">১</span>
                <div class="min-w-0">
                    <p class="text-[13px] font-bold text-slate-900">বামে: সারি / কলাম সংখ্যা → প্রয়োগ</p>
                    <p class="text-xs text-slate-600">টপ কলাম ও সারি লিখে <span class="rounded bg-violet-700 px-1 py-0.5 text-xs font-semibold text-white">প্রয়োগ</span> চাপুন। এই পপআপের <span class="font-semibold text-violet-700">বাম</span> = সেটিংস, <span class="font-semibold text-emerald-700">ডান</span> = লাইভ প্রিভিউ।</p>
                </div>
            </li>
            <li class="flex gap-2.5">
                <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-600 text-[13px] font-bold text-white">২</span>
                <div class="min-w-0">
                    <p class="text-[13px] font-bold text-slate-900">কলাম নাম + সাব-কলাম</p>
                    <p class="text-xs text-slate-600">নাম লিখুন। গ্রুপ হেডারের জন্য <span class="rounded border border-violet-300 bg-violet-50 px-1 py-0.5 text-xs font-semibold text-violet-800">+ সাব</span> চাপুন (যেমন ভ্যাট → প্রযোজ্য / প্রদানকৃত / কম-বেশি)।</p>
                </div>
            </li>
            <li class="flex gap-2.5">
                <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-600 text-[13px] font-bold text-white">৩</span>
                <div class="min-w-0">
                    <p class="text-[13px] font-bold text-slate-900">ডানে সেল ক্লিক → মার্জ</p>
                    <p class="text-xs text-slate-600">ডান প্রিভিউতে সেল <span class="font-semibold text-rose-700">ক্লিক</span> করুন → বামে “নিচে মার্জ” সংখ্যা দিন → <span class="rounded bg-rose-600 px-1 py-0.5 text-xs font-semibold text-white">মার্জ করুন</span>।</p>
                </div>
            </li>
            <li class="flex gap-2.5">
                <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-600 text-[13px] font-bold text-white">৪</span>
                <div class="min-w-0">
                    <p class="text-[13px] font-bold text-slate-900">প্রস্থ (%) ও টেক্সট</p>
                    <p class="text-xs text-slate-600">বামে <span class="font-semibold">প্রস্থ %</span> বদলান · ডানে টাইপ করুন · শেষে উপরের <span class="font-semibold">বন্ধ</span>।</p>
                </div>
            </li>
        </ol>

        <div class="bg-slate-50 px-3 py-2">
            <p class="mb-1.5 text-xs font-bold uppercase tracking-wide text-slate-500">নমুনা ফলাফল — রঙিন নোট = কী দিয়ে বানানো</p>
            ...
            <div class="overflow-x-auto rounded border border-slate-300 bg-white p-1.5">
                <table class="w-full border-collapse text-[11px] leading-tight" style="table-layout: fixed;">
                    <colgroup>
                        <col style="width:14%">
                        <col style="width:10%">
                        <col style="width:22%">
        ...
        <div class="flex flex-wrap items-center gap-2 border-t border-sky-100 bg-white px-3 py-2.5">
            <button
                type="button"
                class="rounded bg-violet-600 px-3 py-1.5 text-[13px] font-semibold text-white hover:bg-violet-700"
                @click="open = false; $wire.applyCustomTableTemplate({{ $bi }}, 'expense')"
            >এই নমুনা কাঠামো লোড করুন</button>
            <p class="text-xs text-slate-500">লোড করলে মার্জ ও সাব-কলাম আগে থেকে থাকবে — তারপর বাম/ডানে বদলান।</p>
        </div>
    </div>
</div>
