...
@foreach ($findings as $fIndex => $finding)
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        @include('livewire.partials.audit-indicator-combobox', [
                            ...
                            'wireKey' => 'p8-ind-'.$fIndex.'-'.(int) ($finding['indicator_id'] ?? 0).'-'.md5((string) ($finding['body'] ?? '')),
                        ])
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page8Findings.{{ $fIndex }}.criteria" rows="2" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        @else
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page8Findings.{{ $fIndex }}.observation" rows="2" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page8Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page8Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[13px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage8StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage8StatsRow({{ $fIndex }})" class="mb-[2mm] text-[13px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif
...
<table class="{{ $tableClass }} min-w-full table-fixed">
                    ...
                    @foreach (($finding['cofRows'] ?? []) as $rowIndex => $row)
                        ...
                        <tr class="{{ $isTotal ? 'font-semibold bg-slate-50' : '' }}">
                                ...
                                <td class="{{ $cellPad }} text-center align-middle">
                                    @if ($editable)
                                        <input
                                            ...
                                            class="w-full border-0 bg-sky-50/50 px-0.5 text-center text-xs {{ $isTotal ? 'font-semibold' : '' }}"
                            ...
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (! $isTotal)
                                        <button type="button" wire:click="removePage8CofRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage8CofRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Month row</button>
        @endif
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[13px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page8Findings.{{ $fIndex }}.risk" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page8Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page8Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} {{ $loop->last ? '' : 'mb-[6mm]' }}">
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page8Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page8Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page8Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[13px]" />
                    @else
