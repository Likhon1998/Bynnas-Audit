...
>
    ...
    <div x-show="!isEditing" class="flex items-start gap-2 rounded border border-slate-200 bg-slate-50/80 px-2 py-2">
        <div class="min-w-0 flex-1">
            <p class="m-0 whitespace-pre-wrap text-[13px] font-medium leading-relaxed text-navy-900" x-text="savedTitle"></p>
            <p x-show="savedCode" class="mt-0.5 m-0 font-mono text-xs text-slate-500" x-text="savedCode"></p>
        </div>
        <button
            type="button"
            @click="startEdit()"
            class="shrink-0 rounded border border-slate-300 bg-white px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-50"
            title="পরিবর্তন"
        >Edit</button>
    </div>
    ...
    <div x-show="isEditing" x-cloak>
        <div class="relative">
            <input
                ...
                class="w-full rounded border border-slate-200 bg-sky-50/40 py-2 pl-2 text-[13px] leading-relaxed focus:border-[#2b579a] focus:ring-[#2b579a]"
            ...
            <div class="absolute right-1.5 top-1/2 flex -translate-y-1/2 items-center gap-1">
                <button
                    ...
                    @click="cancelEdit()"
                    class="rounded border border-slate-200 bg-white px-1.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-50"
                    title="বাতিল"
                >✕</button>
                <button
                    type="button"
                    x-show="allowCreate"
                    @click="commitCustom()"
                    class="rounded bg-[#2b579a] px-2 py-1 text-xs font-semibold text-white hover:bg-[#204072]"
        ...
        >
            <template x-for="(item, idx) in filtered" :key="item.id">
                ...
                >
                    <span class="text-[13px] font-semibold leading-tight text-navy-900" x-text="item.title"></span>
            ...
            <p x-show="!allowCreate && filtered.length === 0 && q.trim().length > 0" class="px-2.5 py-2 text-[13px] text-amber-700">
                মিল পাওয়া যায়নি — Findings Matrix-এর তালিকা থেকে বেছে নিন
            </p>
        </div>
    </div>

    <p class="mt-1 text-xs text-slate-500" x-show="isEditing && allowCreate">
    ...
    <p class="mt-1 text-xs text-slate-500" x-show="!isEditing" x-cloak>
