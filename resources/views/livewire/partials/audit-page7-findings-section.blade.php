...
@foreach ($findings as $fIndex => $finding)
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        @include('livewire.partials.audit-indicator-combobox', [
                            ...
                            'wireKey' => 'p7-ind-'.$fIndex.'-'.(int) ($finding['indicator_id'] ?? 0).'-'.md5((string) ($finding['body'] ?? '')),
                        ])
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page7Findings.{{ $fIndex }}.criteria" rows="4" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        @else
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page7Findings.{{ $fIndex }}.observation" rows="2" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
...
<table class="{{ $tableClass }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page7Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page7Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[13px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage7StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage7StatsRow({{ $fIndex }})" class="mb-[2mm] text-[13px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif
        ...
        @if ($detailType === 'budget' || ($finding['detail_intro'] ?? '') !== '' || $editable)
            @if ($editable)
                <div class="mb-[2mm] flex flex-wrap items-center gap-3">
                    ...
                    <select wire:model.live="page7Findings.{{ $fIndex }}.detail_type" class="rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
                    ...
                    @if ($detailType === 'budget')
                        <label class="text-[13px] font-semibold text-slate-600">বাজেট বছর:</label>
                        <input type="text" wire:model.live="page7Findings.{{ $fIndex }}.budget_year" class="inline-input min-w-[120px] text-[13px]" placeholder="২০২২-২০২৩">
                    @endif
                </div>
            @endif
            ...
            @if ($detailType === 'budget')
                ...
                @if ($editable)
                    <input type="text" wire:model.live="page7Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
                @endif
...
<table class="{{ $tableClass }} mb-[2mm]">
                        ...
                        @foreach (($finding['budgetRows'] ?? []) as $rowIndex => $row)
                            ...
                            <tr class="{{ $isTotal ? 'font-semibold bg-slate-50' : '' }}">
                                    ...
                                    <td class="{{ $cellPad }} {{ $field !== 'budget_head' ? 'text-center' : '' }}">
                                        @if ($editable)
                                            <input
                                                ...
                                                class="w-full border-0 bg-sky-50/50 px-0.5 text-xs {{ $field !== 'budget_head' ? 'text-center' : '' }} {{ $isTotal ? 'font-semibold' : '' }}"
                                ...
                                @if ($editable)
                                    <td class="{{ $cellPad }} text-center">
                                        @if (! $isTotal)
                                            <button type="button" wire:click="removePage7BudgetRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                        @endif
                                    </td>
                                @endif
                            </tr>
                ...
                @if ($editable)
                    <button type="button" wire:click="addPage7BudgetRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Budget row</button>
                @endif
            @endif
...
<table class="{{ $tableClass }} mb-[2mm]">
                            ...
                            <tr>
                                @foreach (['joining_date', 'bonus_date_voucher', 'service_age', 'bonus_amount'] as $field)
                                    <td class="{{ $cellPad }} {{ in_array($field, ['joining_date', 'service_age', 'bonus_amount'], true) ? 'text-center' : '' }}">
                                        @if ($editable)
                                            @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page7Findings.{{ $fIndex }}.bonusRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs text-center" />
                                    @else
                                        <input type="text" wire:model.live="page7Findings.{{ $fIndex }}.bonusRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs text-center">
                                    @endif
                                        @else
                                            {{ $row[$field] ?? '' }}
                                        @endif
                                ...
                                @if ($editable)
                                    <td class="{{ $cellPad }} text-center">
                                        @if (count($finding['bonusRows'] ?? []) > 1)
                                            <button type="button" wire:click="removePage7BonusRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                        @endif
                                    </td>
                                @endif
                            </tr>
                ...
                @if ($editable)
                    <button type="button" wire:click="addPage7BonusRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Bonus row</button>
                @endif
            @endif
        @endif
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[13px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page7Findings.{{ $fIndex }}.risk" rows="3" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page7Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page7Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <table class="{{ $tableClass }} {{ $loop->last ? '' : 'mb-[6mm]' }}">
        <tbody>
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page7Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page7Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page7Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[13px]" />
                    @else
