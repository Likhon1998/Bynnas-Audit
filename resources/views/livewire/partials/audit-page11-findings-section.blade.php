...
@foreach ($findings as $fIndex => $finding)
                ...
                <td class="{{ $cellPad }} align-top">
                    @if ($editable)
                        @include('livewire.partials.audit-indicator-combobox', [
                            ...
                            'wireKey' => 'p11-ind-'.$fIndex.'-'.(int) ($finding['indicator_id'] ?? 0).'-'.md5((string) ($finding['body'] ?? '')),
                        ])
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page11Findings.{{ $fIndex }}.criteria" rows="4" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        @else
    ...
    <div class="mb-[2mm]">
        ...
        @if ($editable)
            <textarea wire:model.live="page11Findings.{{ $fIndex }}.observation" rows="3" class="w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
...
<table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} mb-[2mm]">
                        ...
                        <td class="{{ $cellPad }} text-center">
                            @if ($editable)
                                @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page11Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-transparent text-center text-[13px]" />
                                    @else
                                        <input type="text" wire:model.live="page11Findings.{{ $fIndex }}.statsRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-transparent text-center text-[13px]">
                                    @endif
                            @else
                                {{ $row[$field] ?? '' }}
                            @endif
                    ...
                    @if ($editable)
                        <td class="{{ $cellPad }} text-center">
                            @if (count($finding['statsRows'] ?? []) > 1)
                                <button type="button" wire:click="removePage11StatsRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                            @endif
                        </td>
                    @endif
                </tr>
    ...
    @if ($editable)
        <button type="button" wire:click="addPage11StatsRow({{ $fIndex }})" class="mb-[2mm] text-[13px] font-medium text-[#2b579a]">+ Stats row</button>
    @endif

    @if ($editable)
        <div class="mb-[2mm] flex flex-wrap items-center gap-3">
            <label class="text-[13px] font-semibold text-slate-600">বিস্তারিত ধরন:</label>
            <select wire:model.live="page11Findings.{{ $fIndex }}.detail_type" class="rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
    ...
    @if ($detailType === 'dep_compare')
        ...
        @if ($editable)
            <input type="text" wire:model.live="page11Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]">
        @endif
...
<table class="{{ $tableClass }} min-w-full">
                        ...
                        <tr>
                            @foreach (['asset_group', 'value_report', 'value_register', 'value_diff', 'dep_report', 'dep_register', 'dep_diff'] as $field)
                                <td class="{{ $cellPad }} {{ $field !== 'asset_group' ? 'text-center' : '' }}">
                                    @if ($editable)
                                        @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page11Findings.{{ $fIndex }}.depRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs {{ $field !== 'asset_group' ? 'text-center' : '' }}" />
                                    @else
                                        <input type="text" wire:model.live="page11Findings.{{ $fIndex }}.depRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs {{ $field !== 'asset_group' ? 'text-center' : '' }}">
                                    @endif
                                    @else
                                        {{ $row[$field] ?? '' }}
                                    @endif
                            ...
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['depRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage11DepRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage11DepRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Compare row</button>
        @endif
...
<table class="{{ $tableClass }} min-w-full">
                        ...
                        <tr>
                            @foreach (['product_name', 'product_group', 'purchase_date', 'voucher_no', 'amount', 'quote_status'] as $field)
                                <td class="{{ $cellPad }} {{ in_array($field, ['purchase_date', 'voucher_no', 'amount'], true) ? 'text-center' : '' }}">
                                    @if ($editable)
                                        @if (((string) $field === 'date' || str_ends_with((string) $field, '_date') || preg_match('/^date[_\d]/', (string) $field)))
                                        <x-audit-date-field wire:model.live="page11Findings.{{ $fIndex }}.quoteRows.{{ $rowIndex }}.{{ $field }}" format="dmy" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs {{ in_array($field, ['purchase_date', 'voucher_no', 'amount'], true) ? 'text-center' : '' }}" />
                                    @else
                                        <input type="text" wire:model.live="page11Findings.{{ $fIndex }}.quoteRows.{{ $rowIndex }}.{{ $field }}" class="w-full border-0 bg-sky-50/50 px-0.5 text-xs {{ in_array($field, ['purchase_date', 'voucher_no', 'amount'], true) ? 'text-center' : '' }}">
                                    @endif
                                    @else
                                        {{ $row[$field] ?? '' }}
                                    @endif
                            ...
                            @if ($editable)
                                <td class="{{ $cellPad }} text-center">
                                    @if (count($finding['quoteRows'] ?? []) > 1)
                                        <button type="button" wire:click="removePage11QuoteRow({{ $fIndex }}, {{ $rowIndex }})" class="text-xs text-rose-600">×</button>
                                    @endif
                                </td>
                            @endif
                        </tr>
        ...
        @if ($editable)
            <button type="button" wire:click="addPage11QuoteRow({{ $fIndex }})" class="mb-[3mm] text-[13px] font-medium text-[#2b579a]">+ Quote row</button>
        @endif
    @elseif (($finding['detail_intro'] ?? '') !== '' && ! $editable)
        <p class="mb-[2mm] font-semibold">{{ $finding['detail_intro'] }}</p>
    @elseif ($editable && $detailType === 'none')
        <input type="text" wire:model.live="page11Findings.{{ $fIndex }}.detail_intro" class="mb-[2mm] w-full rounded border border-slate-200 bg-sky-50/40 px-2 py-1 text-[13px]" placeholder="বিস্তারিত নিম্নে দেওয়া হলো:">
    @endif
    ...
    <div class="mb-[2mm] space-y-[2mm] text-[13px] leading-relaxed">
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page11Findings.{{ $fIndex }}.risk" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
            @else
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page11Findings.{{ $fIndex }}.root_cause" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
        ...
        <div>
            ...
            @if ($editable)
                <textarea wire:model.live="page11Findings.{{ $fIndex }}.recommendation" rows="2" class="mt-1 w-full rounded border border-slate-200 bg-sky-50/40 p-2 text-[13px]"></textarea>
    ...
    <table class="{{ $compact ? 'a4-table a4-table-compact text-xs' : 'w-full border-collapse text-[10.5px]' }} {{ $loop->last ? '' : 'mb-[6mm]' }}">
        <tbody>
            <tr>
                <td class="{{ $cellPad }} w-[38%] font-semibold align-top">শাখা ব্যবস্থাপকের জবাব</td>
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page11Findings.{{ $fIndex }}.bm_reply" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['bm_reply'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <textarea wire:model.live="page11Findings.{{ $fIndex }}.responsible" rows="2" class="w-full border-0 bg-sky-50/40 p-1 text-[13px]"></textarea>
                    @else
                        {{ $finding['responsible'] ?? '' }}
                    @endif
                </td>
            ...
            <tr>
                ...
                <td class="{{ $cellPad }}">
                    @if ($editable)
                        <x-audit-date-field wire:model.live="page11Findings.{{ $fIndex }}.resolution_date" format="dmy" class="w-full border-0 bg-sky-50/40 px-1 text-[13px]" />
                    @else
