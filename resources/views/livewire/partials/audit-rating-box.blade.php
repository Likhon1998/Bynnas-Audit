...
<table class="w-full border-collapse text-xs">
    <tr>
        <td colspan="2" class="border border-black bg-[#4472C4] p-[1.5mm] text-center text-xs font-bold text-white">
            রেটিং (Rating)
        </td>
    </tr>
    <tr>
        @if ($editable && $wireModel)
            <td colspan="2" class="border border-black p-[1.5mm] text-center font-bold" style="background: {{ $cellBg }}; color: {{ $cellColor }};">
                <select
                    ...
                    class="w-full border-0 bg-transparent text-center text-xs font-bold"
        ...
        @else
            ...
            <td class="w-1/2 border border-black p-[1.5mm] text-center text-[13px] font-bold" style="background: {{ $cellBg }}; color: {{ $cellColor }};">{{ $code }}</td>
        @endif
    </tr>
</table>
