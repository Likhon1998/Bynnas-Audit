...
<x-app-layout>
    <div class="px-3 py-3 lg:px-5" style="font-family:'Hind Siliguri','Nirmala UI',system-ui,sans-serif;">
        ...
        <div class="mb-3 rounded-xl border border-slate-200 bg-slate-50 px-3.5 py-2.5">
            ...
            @if ($canStepIn)
                <a href="{{ route('audit-review.show', $report) }}" class="mt-2 inline-flex h-8 items-center rounded-md bg-navy-900 px-3 text-[13px] font-semibold text-white hover:bg-slate-800">
        ...
        <div class="mb-3 flex flex-wrap gap-2">
            <span class="inline-flex rounded-md border px-2.5 py-1 text-[13px] font-semibold {{ $positionTone[$posKey] ?? 'border-slate-200 bg-white text-slate-700' }}">
                {{ $report->workflowPositionLabel() }}
            </span>
            @if ($report->memo_no)
                <span class="inline-flex rounded-md border border-slate-200 bg-white px-2.5 py-1 text-[13px] text-slate-600">Memo {{ $report->memo_no }}</span>
            @endif
        </div>
