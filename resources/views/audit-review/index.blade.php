                ...
                @foreach ($actionNotes as $note)
                    ...
                    <div class="rounded-xl border px-3.5 py-3 shadow-sm {{ $tone['card'] }}">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            @if (($note['count'] ?? 0) > 0)
                                <span class="inline-flex h-5 min-w-5 items-center justify-center rounded-full px-1.5 text-xs font-bold {{ $tone['badge'] }}">{{ $note['count'] }}</span>
                            @endif
                        </div>
                        <p class="mt-1 text-[12px] leading-relaxed {{ $tone['body'] }}">{{ $note['body'] }}</p>
                        <a href="{{ $note['url'] }}" class="mt-2.5 inline-flex h-8 items-center rounded-md px-2.5 text-[13px] font-semibold {{ $tone['btn'] }}">
                            {{ $note['action'] }}
                        </a>
                    </div>
                @endforeach
        ...
        @elseif ($clearNote)
            <div class="mb-3">
                ...
                >
                    ...
                    <span class="text-[13px] font-medium text-emerald-700/80">· refresh</span>
                </a>
            </div>
        @endif
            ...
            @foreach ([
                ...
                >
                    ...
                    @if ($meta['count'] !== null && $meta['count'] > 0)
                        <span class="inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-xs font-bold {{ $tab === $key ? 'bg-white/20 text-white' : 'bg-rose-100 text-rose-700' }}">{{ $meta['count'] }}</span>
                    @endif
                </a>
            @endforeach
        </div>
                        ...
                        @forelse ($reports as $report)
                            ...
                            <tr class="hover:bg-sky-50/40">
                                ...
                                <td class="px-3 py-2.5 whitespace-nowrap">
                                    ...
                                    <span class="inline-flex rounded-full px-2 py-0.5 text-xs font-semibold {{ $round >= 2 ? 'bg-amber-50 text-amber-800' : 'bg-violet-50 text-violet-800' }}">
                                ...
                                <td class="px-3 py-2.5 text-[13px] font-medium text-slate-700">{{ $todo }}</td>
                                ...
                                <td class="px-3 py-2.5 text-right">
                                    <div class="inline-flex flex-wrap items-center justify-end gap-2">
                                        @if ($tab === 'inbox' && $canActRow)
                                            <a href="{{ route('audit-review.show', $report) }}" class="inline-flex h-7 items-center rounded-md bg-emerald-700 px-2.5 text-[13px] font-semibold text-white hover:bg-emerald-800">Open to edit</a>
                                            <form
                                                method="POST"
                                            ...
                                            >
                                                @csrf
                                                <button type="submit" class="inline-flex h-7 items-center rounded-md bg-emerald-600 px-2.5 text-[13px] font-semibold text-white hover:bg-emerald-700">Totally fixed</button>
                                            </form>
                                            <details class="relative">
                                                <summary class="cursor-pointer list-none text-[13px] font-semibold text-rose-700 hover:underline [&::-webkit-details-marker]:hidden">Return</summary>
                                                <form
                                                    method="POST"
                                                    action="{{ route('audit-review.request-changes', $report) }}"
                                                    class="absolute right-0 z-20 mt-1 w-56 rounded-lg border border-slate-200 bg-white p-2 shadow-lg"
                                                ...
                                                >
                                                    @csrf
                                                    <textarea name="body" rows="3" required class="mb-2 w-full rounded-md border-slate-200 text-[13px]" placeholder="What should the maker fix?"></textarea>
                                                    <button type="submit" class="inline-flex h-8 w-full items-center justify-center rounded-md bg-rose-700 text-[13px] font-semibold text-white hover:bg-rose-800">Return to maker</button>
                                                </form>
                                            </details>
                                        @elseif ($tab === 'returned')
                                            <a href="{{ route('audits.index', ['report' => $report->id]) }}" class="inline-flex h-7 items-center rounded-md bg-[#2b579a] px-2.5 text-[13px] font-semibold text-white hover:bg-[#204072]">Edit report</a>
                                            <a href="{{ route('audit-review.show', $report) }}" class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-[13px] font-semibold text-slate-700 hover:bg-slate-50">View comments</a>
                                        @elseif ($isReadyRow)
                                            <a href="{{ route('audit-review.show', $report) }}" class="inline-flex h-7 items-center rounded-md bg-emerald-700 px-2.5 text-[13px] font-semibold text-white hover:bg-emerald-800">Edit marks</a>
                                            <form
                                                method="POST"
                                            ...
                                            >
                                                @csrf
                                                <button type="submit" class="inline-flex h-7 items-center rounded-md bg-[#2b579a] px-2.5 text-[13px] font-semibold text-white hover:bg-[#204072]">Send to maker</button>
                                            </form>
                                            <form
                                                method="POST"
                                            ...
                                            >
                                                @csrf
                                                <button type="submit" class="inline-flex h-7 items-center rounded-md bg-emerald-700 px-2.5 text-[13px] font-semibold text-white hover:bg-emerald-800">Confirm</button>
                                            </form>
                                            <form
                                                method="POST"
                                                action="{{ route('audit-review.totally-fixed', $report) }}"
                                            ...
                                            >
                                                @csrf
                                                <button type="submit" class="inline-flex h-7 items-center rounded-md bg-teal-600 px-2.5 text-[13px] font-semibold text-white hover:bg-teal-700">Totally fixed</button>
                                            </form>
                                        @elseif ($canReopen)
                                            <form
                                                method="POST"
                                            ...
                                            >
                                                @csrf
                                                <button type="submit" class="inline-flex h-7 items-center rounded-md bg-amber-600 px-2.5 text-[13px] font-semibold text-white hover:bg-amber-700">Reopen</button>
                                            </form>
                                            <a href="{{ route('audit-review.show', $report) }}" class="text-[13px] font-semibold text-[#2b579a] hover:underline">Open</a>
                                        @else
                                            <a href="{{ route('audit-review.show', $report) }}" class="text-[13px] font-semibold text-[#2b579a] hover:underline">Open</a>
                                        @endif
                                    </div>
